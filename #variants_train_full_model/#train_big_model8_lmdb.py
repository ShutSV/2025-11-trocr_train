...
from datasets_rus_lmdb import MODEL_NAME, processor, train_dataset, val_dataset
...
def main():
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        eval_strategy="steps",
        eval_steps=3000,    # Частая валидация для мониторинга
        save_strategy="steps",
        save_steps=3000,
        save_total_limit=3,
        ...
        dataloader_num_workers=8,    # Параллельная загрузка - ОТКЛЮЧИТЬ для [i9 185H]
        group_by_length=True,  # Рекомендуется включить, так как много RAM для кеширования батчей
    )
    ...
    # Добавим кастомный вывод RAM перед стартом
    print(f"✅ Готов к старту. {train_dataset.get_ram_stats()}")
